...
implicit none
save
private :: chgm,sgamma!factorial,gamma
contains
!=======================================================================================================================!
!                                                       THE BOYS FUNCTION                                               !
!=======================================================================================================================!
...
!                                                                                                                       !
!                                                                                                                       !
!                                           A FUNCTION AND 2 AUXILLIARY SUBROUTINES                                     !
!                                                                                                                       !
!                                                                                                                       !
!                                                                                                                       !
!=======================================================================================================================!

    function boys(n,t) result(b)
    integer , intent(in) :: n
    real (kind = 8) hg,td0
    real (kind = 8) ::t,b,pi=3.141592653,l
        if (t < 20) then
            td0 = t
            call chgm( n+0.5d0 , n+1.5d0 , -td0, hg)
            b = hg/(2.0*n + 1.0)
        else
            !if (n == 0) then
                !b = 0.5*(pi/t**(2*n+1))**0.5
            !else
                !b = factorial(2*n-1)/(factorial(2*n-1)*2**(2*n))
                !b = b * (pi/t**(2*n+1))**0.5
                !b = factorial(2*n)/(factorial(n)*2**(2*n+1))
                b = real(odd_factorial(2*n-1))/(2**(n+1))
                b = b * (pi/t**(2*n+1))**0.5
            !endif
        endif
        !print *, b
    ...
    recursive real function rboys(n,x) result(b)
    integer , intent(in) :: n
    real (kind = 8) , intent(in) :: x
    real (kind = 8) :: pi=3.141592653,l
        !print *, t
        if (x< 1.1) then
            b = 1/(2*n+1)  
        else if (x < 20) then
            if (n == 0) then
                b = (pi / (4 * x))**0.5 * erf( x**0.5 )
            else
                !b = ( (2*n-1) * rboys(n-1,x) - exp(-x) ) / (2 * x)
                b = ( (2*x + 2*n - 1)*rboys(n-1,x) - (2*n - 3)*rboys(n-2,x) ) / (2 * x)
            endif
        else
            b = real(odd_factorial(2*n-1))/(2**(n+1))
            b = b * (pi/x**(2*n+1))**0.5  
    ...
    recursive integer function factorial(i) result(f)  
    integer , intent(in) :: i
        if (i==0) then
            f=1 
        else
    ...
    recursive integer function odd_factorial(i) result(f)
    integer , intent(in) :: i
        if (i < -1) then
            print *, 'integer smaller than -1 is given: exit odd_factorial evaluation and abort program'
            stop
        endif
        if (i == -1) then
            f = 1
        else
            f = i*odd_factorial(i-2)
        endif
    end function
